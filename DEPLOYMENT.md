...
2. Auto-generate environment variables (`MYSQL_URL`, `MYSQL_PUBLIC_URL`, dll)
3. Auto-detect Node.js dan build commands
4. Built-in web terminal untuk migrations
5. Satu dashboard untuk semua
6. Auto-shared variables antar services

**📝 Project ini menggunakan Railway Database dengan environment variables:**

- `MYSQL_URL` - Untuk koneksi internal (backend → database)
- `MYSQL_PUBLIC_URL` - Untuk koneksi eksternal (local → database)
- `DATABASE_URL=${{MYSQL_URL}}` - Set di backend service
...
#### Option 3: Vercel (Backend) + Railway (Database) ⭐ RECOMMENDED

✅ **Rekomendasi untuk project ini!**

- 🆓 **Vercel**: Free tier untuk backend (serverless)
- 🗄️ **Railway**: Database MySQL dengan auto-generated variables
...
#### Option 3B: Vercel (Backend) + Hostinger (Database)

✅ **Alternatif jika sudah punya hosting Hostinger!**

- 🆓 **Vercel**: Free tier untuk backend (serverless)
- 💰 **Hostinger**: Pakai database yang sudah ada
- 📝 **Panduan Lengkap**: Lihat Option 3B di bagian Backend Deployment
...
3. **Add Postgres Service (or use Supabase)**

   - Click "New" > "Database" > "Add Postgres" (Railway) or create a project on Supabase
   - Railway/Supabase will automatically create a Postgres database
   - Platform akan otomatis generate environment variables berikut:

4. **Environment Variables yang Disediakan Railway**

   Setelah service Postgres dibuat, platform otomatis menyediakan environment variables serupa:

   **Database Connection Variables (contoh):**
   ...
   **Penjelasan:**
   ...
   - `POSTGRES_URL`: Untuk koneksi internal (dari service lain di platform yang sama)
   - `POSTGRES_PUBLIC_URL`: Untuk koneksi eksternal (dari luar, seperti local machine)
   - `POSTGRES_PASSWORD`: Password auto-generated oleh platform (simpan dengan aman!)
   - `RAILWAY_PRIVATE_DOMAIN`: Domain internal Railway untuk koneksi antar service (jika menggunakan Railway)
   - `RAILWAY_TCP_PROXY_DOMAIN`: Domain public untuk koneksi eksternal (Railway)

5. **Cara Menggunakan di Backend**

   **Opsi 1: Gunakan POSTGRES_URL (Recommended untuk backend di platform yang sama)**

   - Jika backend juga di-deploy di platform yang sama (Railway/Supabase), gunakan `POSTGRES_URL`
   - Platform otomatis resolve variable references (`${{...}}`)
   - Set di backend service: `DATABASE_URL=${{POSTGRES_URL}}`

   **Opsi 2: Gunakan POSTGRES_PUBLIC_URL (Untuk koneksi dari luar)**

   - Jika perlu connect dari local machine (untuk migrations, dll)
   - Copy value dari dashboard (sudah di-resolve)
   - Format: `postgresql://postgres:password@host:5432/railway?schema=public&sslmode=require`

**Keuntungan Railway:**

- ✅ Free tier yang generous ($5 credit/bulan)
- ✅ Auto-scaling
- ✅ Auto-generate connection strings
- ✅ Easy connection dengan backend di Railway (shared variables)
- ✅ Built-in monitoring
- ✅ Private domain untuk koneksi internal yang lebih secure
...
### Option 1: Render (Backend) + Railway (Database)

**📝 Project ini menggunakan Railway Database**

1. **Setup Database di Railway** (Lakukan dulu)

   - Ikuti panduan di **Option 3: Railway Database** di bagian Database Setup
   - Setelah MySQL service dibuat, Railway akan generate environment variables
   - Catat `MYSQL_PUBLIC_URL` dari Railway dashboard (untuk koneksi eksternal)

2. **Create Account di Render**
...
3. **Create Web Service**
   ...
   - Connect your repository: `milestone-4-whtrianto`
   - Configure:
     - **Name**: revobank-api
     - **Environment**: Node
...
4. **Environment Variables**
   ...
   ```
   DATABASE_URL=<copy-MYSQL_PUBLIC_URL-dari-Railway>
   ```

   **Cara mendapatkan `MYSQL_PUBLIC_URL`:**

   - Login ke Railway dashboard
   - Klik pada MySQL service
   - Pergi ke tab **"Variables"**
   - Copy value dari **`MYSQL_PUBLIC_URL`** (bukan `MYSQL_URL`, karena Render di luar Railway)
   - Format: `mysql://root:password@railway-tcp-proxy-domain:port/railway`
...
4. **Configure Environment Variables** (2 menit)
   ...
   **Jika menggunakan Railway Database (Recommended):**

   Railway otomatis generate environment variables untuk MySQL. Untuk connect backend ke database:

   - Di Railway dashboard, klik pada **backend service** (bukan MySQL service)
   - Pergi ke tab **"Variables"**
   - Railway sudah otomatis share variables dari MySQL service
   - **Tinggal set `DATABASE_URL`** dengan reference ke MySQL:

     ```
     DATABASE_URL=${{MYSQL_URL}}
     ```

     - Railway akan otomatis resolve `${{MYSQL_URL}}` menjadi connection string lengkap
     - Format resolved: `mysql://root:password@railway-private-domain:3306/railway`

   - Tambahkan variable lainnya:
     ```
   ...
   **Jika menggunakan Hostinger Database:**

   - Tambahkan `DATABASE_URL` manual:
     ```
     DATABASE_URL=mysql://user:password@mysql.hostinger.com:3306/database
     ```
   - Tambahkan variable lainnya seperti di atas

   **Tips**:

   - Generate JWT_SECRET di https://randomkeygen.com/ atau pakai:
     ```bash
     openssl rand -base64 32
     ```
   - Untuk melihat resolved value `MYSQL_URL`, klik pada MySQL service > Variables tab
...
6. **Run Migrations** (2 menit)
   ...
   **Opsi 1: Menggunakan Railway Web Terminal (Recommended)**
   ...
   - Setelah deploy selesai, buka **Railway web terminal**:
     - Klik pada **backend service** (bukan MySQL service)
   ...
   - Di terminal, jalankan:
     ...
     npm run prisma:migrate deploy
     ```
   - Railway sudah otomatis set `DATABASE_URL=${{MYSQL_URL}}`, jadi migrations akan langsung connect ke database
   ...
   **Opsi 2: Menggunakan Railway CLI**
   ...
   # Run migrations (Railway otomatis inject environment variables)
   railway run npm run prisma:migrate deploy
   ```

   **Opsi 3: Dari Local Machine (Jika perlu)**

   - Jika perlu run migrations dari local machine:
     - Di Railway dashboard, klik pada **MySQL service**
     - Pergi ke tab **"Variables"**
     - Copy value dari **`MYSQL_PUBLIC_URL`** (bukan `MYSQL_URL`, karena itu untuk internal)
     - Di local machine, set environment variable:

       ```bash
       # Windows PowerShell
       $env:DATABASE_URL="mysql://root:password@railway-tcp-proxy-domain:port/railway"

       # Linux/Mac
       export DATABASE_URL="mysql://root:password@railway-tcp-proxy-domain:port/railway"
       ```
...
7. **Seed Database (Optional)**
   ...
   **Menggunakan Railway Web Terminal:**

   - Buka terminal di backend service
   - Jalankan:
     ```bash
     npm run prisma:seed
     ```
   ...
   **Dari Local Machine (Jika perlu):**

   - Gunakan `MYSQL_PUBLIC_URL` dari Railway dashboard
   - Set `DATABASE_URL` di local machine
   - Run: `npm run prisma:seed`

**✅ Selesai!** API Anda sudah live di URL yang diberikan Railway (format: `https://your-app-name.up.railway.app`)

**📝 Catatan Penting tentang Railway Environment Variables:**

1. **Variable References (`${{...}}`)**:

   - Railway menggunakan syntax `${{VARIABLE_NAME}}` untuk reference variables
   - Railway otomatis resolve references saat runtime
   - Contoh: `DATABASE_URL=${{MYSQL_URL}}` akan di-resolve menjadi connection string lengkap

2. **MYSQL_URL vs MYSQL_PUBLIC_URL**:

   - **`MYSQL_URL`**: Untuk koneksi internal (backend di Railway → database di Railway)
     - Menggunakan `RAILWAY_PRIVATE_DOMAIN` (lebih secure, lebih cepat)
     - Format: `mysql://root:password@railway-private-domain:3306/railway`
     - **Gunakan ini untuk backend di Railway** (set `DATABASE_URL=${{MYSQL_URL}}`)
   - **`MYSQL_PUBLIC_URL`**: Untuk koneksi eksternal (local machine → database di Railway)
     - Menggunakan `RAILWAY_TCP_PROXY_DOMAIN` (accessible dari internet)
     - Format: `mysql://root:password@railway-tcp-proxy-domain:port/railway`
     - **Gunakan ini untuk migrations dari local machine**

3. **Environment Variables yang Disediakan Railway MySQL**:
...
4. **Auto-Shared Variables**:
   - Railway otomatis share variables dari MySQL service ke service lain di project yang sama
   - Tidak perlu manual copy-paste connection string
   - Cukup set `DATABASE_URL=${{MYSQL_URL}}` di backend service
...
- ✅ Auto-shared environment variables antar services
- ✅ Private domain untuk secure internal connections
